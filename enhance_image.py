...
favicon = './Favi.png'
st.set_page_config(page_title='AquaEnhance Web-App "Underwater Image Enhancement Made Easy', page_icon=favicon)
...
def main():
    selected_box = st.sidebar.selectbox('Options', ('AquaEnhance', 'About the App'))
    if selected_box == 'About the App':
        about()
    if selected_box == 'AquaEnhance':
        image_enhancer()

def about():
    st.title("Welcome to AquaEnhance!")
    st.caption("Underwater Image Enhancer Web App")
    with st.expander("Enhance Process"):
        st.image('./images/block_diagram.png', use_column_width=True)
    with st.expander("About AquaEnhancer"):
        st.write("""we believe that your underwater images deserve to be clear, vibrant, and full of 
        detail. That's why we've developed a user-friendly image enhancer that uses advanced AI techniques to bring 
        out the best in your photos. With just a few clicks, you can enhance your underwater photography and make it 
        stand out. Try AquaEnhance today and see the difference for yourself!""")
    ...
    with st.expander("Team Members"):
        st.write("""Manoj Pandian G V
                    \nSurendar M
                    \nBharth M
                    \nRaja Pandi L""")

def image_enhancer():
    st.header("AquaEnhancer Web App")
...
def download_button(img_array, file_name):
    # Create download button with Base64 encoded image data
    from io import BytesIO
    import base64
    ...
    dl_button = (
        f'<a href="data:file/png;base64,{base64.b64encode(buffer.getvalue()).decode()}" '
        f'download="{file_name}.png">Download {file_name}</a>'
    )
    return dl_button
    ...
    if file is None:
        ...
        if image.mode != 'RGB':
            ...
            if st.button("Enhance Uploaded Image"):
                ...
                st.text("Enhanced Image Using PCA Based Fusion")
                st.image(pcafused, use_column_width=True)
                output_pca = get_image_download_link(pcafused, "PCA_fused_image", "Download PCA fused image")
                st.markdown(output_pca, unsafe_allow_html=True)
                st.text("Enhanced Image Using Averaging Based Fusion")
                st.image(averagefused, use_column_width=True)
                output_avg = get_image_download_link(averagefused, "Average_fused_image", "Download Averaging fused "
                                                                                          "image")
                st.markdown(output_avg, unsafe_allow_html=True)
